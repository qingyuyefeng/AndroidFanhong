...
public class BluetoothClientService extends Service {
	...
	private BroadcastReceiver controlReceiver = new BroadcastReceiver() {
		...
		/*		class MyRunnable implements Runnable{
				...
				t.start();*/
			} else if (BluetoothTools.ACTION_ACL_DISCONNECTED.equals(action)) {
				//断开蓝牙连接
				communSocket.close();
				communSocket = null;
			...
			} else if (BluetoothTools.ACTION_RECEIVE_FILE.equals(action)) {
				Bundle bundle = intent.getExtras();
				final long length = bundle.getLong("length");
				final int frame = bundle.getInt("frame");
				Log.i("hu","服务收到 length="+length+" frame="+frame);
				if (communSocket != null) {
					class MyRunnable1 implements Runnable{
						public void run(){
							communSocket.receiveFile(length,frame);
						}
					}
	...
	*/
		...
		public void handleMessage(Message msg) {
			...
			switch (msg.what) {
				case BluetoothTools.MESSAGE_CONNECT_ERROR://连接错误
					...
					break;
				case BluetoothTools.MESSAGE_CONNECT_BREAK:
					//连接断开
					Intent errorIntent1 = new Intent(BluetoothTools.ACTION_CONNECT_BREAK);
					sendBroadcast(errorIntent1);
					break;
				case BluetoothTools.MESSAGE_CONNECT_SUCCESS://连接成功
					...
					//	communSocket.start();
					//发送连接成功广播
					Log.i("hu","开启通讯线程**********BluetoothClientService.java p135");
					Intent succIntent = new Intent(BluetoothTools.ACTION_CONNECT_SUCCESS);
					//	succIntent.putExtra("deviceName", (Serializable)device.getName());
					//	succIntent.putExtra(BluetoothTools.DEVICE, device);
				...
				case BluetoothTools.FILE_RECIVE_PERCENT://文件接收百分比
					...
					break;
				case BluetoothTools.PHONE_SHOW_BUFFER:
					Intent flieIntent2 = new Intent(BluetoothTools.ACTION_SHOW_BUFFER);
					flieIntent2.putExtra(BluetoothTools.DATA, (String)msg.obj);
					sendBroadcast(flieIntent2);
					break;
			}
	...
	public void onCreate() {
		...
		controlFilter.addAction(BluetoothTools.ACTION_ACL_DISCONNECTED);
		controlFilter.addAction(BluetoothTools.ACTION_RECEIVE_FILE);
		//注册BroadcastReceiver
